...
@Client.on_message(command(["play", f"play@{BOT_USERNAME}", "شغل", "مستر", "تشغيل", "مجك", "موسيقي", "اللواء", "محمد"]) & other_filters)
async def play(c: Client, m: Message):
    ...
    keyboard = InlineKeyboardMarkup(
                  [[
                      ...
                      InlineKeyboardButton("• المطور", url=f"https://t.me/MR_X_N"),
                      InlineKeyboardButton("• قـنـأة ألـسـوُرس", url=f"https://t.me/MR_X_N_2"),
                  ],[
    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo=f"{IMG_1}",
                    caption=f"💡 **تمت إضافة المسار إلى قائمة الانتظار »** `{pos}`\n\n🏷 **اسـم:** [{songname}]({link}) | `music`\n💭 **شـات:** `{chat_id}`\n🎧 **مـطـلـوبه بواسـطـة:** {m.from_user.mention()}",
             ...
             try:
                ...
                await m.reply_photo(
                    photo=f"{IMG_2}",
                    caption=f"🏷 **اسـم:** [{songname}]({link})\n💭 **شـات:** `{chat_id}`\n💡 **الـحـاله:** `Playing`\n🎧 **مـطـلـوبه بواسـطـه:** {requester}\n📹 **Stream type:** `Music`",
                    reply_markup=keyboard,
                )
             except Exception as e:
                await suhu.delete()
            ...
            if search == 0:
                ...
                if abhi == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        await m.reply_photo(
                            photo=image,
                            caption=f"💡 **تمت إضافة المسار إلى قائمة الانتظار »** `{pos}`\n\n🏷 **اسـم:** [{songname[:22]}]({url}) | `music`\n**⏱ الـوقـت:** `{duration}`\n🎧 **مـطـلوبـه بـواسـطـة:** {requester}",
                            reply_markup=keyboard,
                        )
                    else:
                        try:
                            ...
                            await m.reply_photo(
                                photo=image,
                                caption=f"🏷 **اسـم:** [{songname[:22]}]({url})\n**⏱ الوقـت:** `{duration}`\n💡 **الـحـالـه:** `Playing`\n🎧 **مـطـلـوبـه بـواسـطـة:** {requester}",
                                reply_markup=keyboard,
                            )
